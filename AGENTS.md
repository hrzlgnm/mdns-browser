...
- The backend HTTP client must use rustls, not native-tls, because native-tls (openssl) cannot cross-compile to Android. Keep `reqwest` on `default-features = false, features = ["rustls-tls"]`. On `reqwest` 0.12 this resolves to webpki roots with the `ring` provider; avoid feature sets that pull `aws-lc-rs` or `rustls-platform-verifier`, which need a C toolchain and extra JNI setup on Android.
