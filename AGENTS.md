...
cargo --locked tauri android build --config src-tauri/tauri.android.conf.json
```
...
- The backend HTTP client must use rustls, not native-tls, because native-tls (openssl) cannot cross-compile to Android. Keep `tauri-plugin-http` on `default-features = false, features = ["rustls-tls"]`.
